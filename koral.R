...
library(writexl)
library(lubridate)
setwd("D:/Ankit Work/wind data/Koral")
A<- read_excel("KoralJune1_17July.xlsx",sheet = "KoralJune1_July17")
###### daily #############
wind <- A[, c(1,3)]
n <- 96;
...
write_xlsx(df_new,"D:/Ankit Work/wind data/koral_daily.xlsx")
#######
library(dplyr)
...
n<- 4;
wind_avg=aggregate(wind,list(rep(1:(nrow(wind)%/%n+1),each=n,len=nrow(wind))),mean)[-1]
tt<- seq(as.POSIXct("2020-06-01 01:00:00"), as.POSIXct("2020-07-18 00:00:00"), by="hour")
...
write_xlsx(df2,"D:/Ankit Work/wind data/koral_horly.xlsx")
#######
library(dplyr)
new_df=merge(act_avg,N25_avg,AN12_avg,CWI_avg,ECM9_avg,by=1)
df=do.call("rbind", list(act_avg,N25_avg,AN12_avg,CWI_avg,ECM9_avg))
